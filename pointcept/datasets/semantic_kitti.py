...
class SemanticKITTIDataset(Dataset):
    ...
    def prepare_train_data(self, idx):
        ...
        # 0.01%
        if self.weak:
            scene_num = label_file.split('/')[-3]
            frame_num = label_file.split('/')[-1]
            weak_path = '/mnt/jihun/semantic_kitti/weak_label'
            segment = segment*(np.fromfile(os.path.join(weak_path,scene_num,frame_num), dtype=np.int64) !=-1)
            
            # weak_data = np.zeros_like(segment)
            # unique_labels = np.unique(segment)
            # for unique_lab in unique_labels:
            #     label_index = np.where(segment==unique_lab)[0]
            #     label_nums = label_index.shape[0]
            #     selected_index = int(label_nums/2)
            #     weak_data[label_index[selected_index]] = segment[label_index[selected_index]]
            # segment = weak_data
        # import pdb;pdb.set_trace()
        ...
        #         label_index = np.where(segment==unique_lab)[0]
        #         label_nums = label_index.shape[0]
        #         selected_index = [int((i+1)*label_nums/11) for i in range(10)]
        #         weak_data[label_index[selected_index]] = segment[label_index[selected_index]]
        #     segment = weak_data
