...
class Trainer(TrainerBase):
    ...
    def run_step(self):
        if self.epoch%5==0:
            ...
            with torch.cuda.amp.autocast(enabled=self.cfg.enable_amp):
                output_dict, prompt_dict = self.model(input_dict, epoch=self.epoch)
            ...
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.cuda.amp.autocast(enabled=self.cfg.enable_amp):
                output_dict = self.model(input_dict, epoch=self.epoch)
                loss = output_dict["loss"]
            self.optimizer.zero_grad()
            if self.cfg.enable_amp:
                self.scaler.scale(loss).backward()
    ...
    def after_epoch(self):
        ...
        if self.epoch%5==0:
            ...
            pickle_path = "/home/vilab/khj/ssd0/pointcept/pickle_path/"+str(local_rank)+'.pickle'
            ...
            if comm.is_main_process():
                ...
                dict_full = {}
                for ridx in range(4):
                    pickle_path = "/home/vilab/khj/ssd0/pointcept/pickle_path/"+str(ridx)+'.pickle'
                    with open(pickle_path, 'rb') as pp:
                        dict_temp = pickle.load(pp)
                    dict_full = {**dict_full, **dict_temp}  

                for scene_id, prompts in dict_full.items():
                    ...
                    if weak_idx.shape[0]!=0:
                        ...
                        # coord = torch.load('/home/vilab/khj/ssd0/pointcept/data/s3dis/Area_6/hallway_5.pth')['coord']
