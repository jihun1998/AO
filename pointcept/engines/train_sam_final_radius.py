...
class Trainer(TrainerBase):
    def __init__(self, cfg):
        ...
        self.log_dir = '/mnt/jihun4/pointceptHJ/'+self.cfg.save_path
                 
        pickle_path = '/mnt/jihun4/pointceptHJ/data/basket_s3dis.pickle'
        with open(pickle_path, 'rb') as pp:
            self.basket = pickle.load(pp)
        print('Building basket done')
    ...
    def run_step(self):
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg.enable_amp):
            output_dict, seg_dict = self.model(input_dict)
            loss = output_dict["loss"]
            for k,v in seg_dict.items():
                seg = v[0]
                ori_idx = v[1]
                self.basket[k][ori_idx.cpu().detach().numpy()] = seg.cpu().detach().numpy()
    ...
    def after_epoch(self):
        ...
        pickle_root = '/mnt/jihun4/pointceptHJ/pickle_path'

        if not comm.is_main_process():
            local_rank = comm.get_local_rank()
            pickle_path = pickle_root+'/'+str(local_rank)+'_prompt.pickle'
            with open(pickle_path, 'wb') as pp:
                pickle.dump(self.basket, pp, protocol=pickle.HIGHEST_PROTOCOL)
            ...
            torch.distributed.barrier() # multi-gpu
            print('main awake')

        if comm.is_main_process():
            pickle_paths = glob.glob(pickle_root+'/*.pickle')
            for pickle_path in pickle_paths:
                ...
                for k,v in dict_temp.items():
                    mask_temp = v!=-100
                    self.basket[k][mask_temp] = v[mask_temp]
                del dict_temp
                       
            print("Aggregation across ddp done")
            
            # Roots
            data_3d_root = '/mnt/jihun4/pointceptHJ/data/s3dis'
            cls_gt_root = '/mnt/jihun4/pointceptHJ/data/s3dis_cls_gt'
            rgb_2d_root = '/mnt/jihun4/pointceptHJ/data/S2D3D'
            bridge_root = '/mnt/jihun4/pointceptHJ/data/bridge'
            bridge_root2 = '/mnt/jihun4/pointceptHJ/data/bridge2'
            
            embedding_root = '/mnt/jihun4/pointceptHJ/data/embeddings'
            embedding_root2 = '/mnt/jihun4/pointceptHJ/data/embeddings2'
            ...
            for ridx, (scene_id, seg_logit) in enumerate(tqdm.tqdm(self.basket.items())):

                flag_updated = False
                
                # Paths
                temp = scene_id.split('_')
                area_str = temp[2]+'_'+temp[3]
                room_str = temp[4]+'_'+temp[5]

                rgb_path_all = rgb_2d_root + '/' + area_str + '/data/rgb'
                bridge_paths = sorted(glob.glob(bridge_root+'/'+area_str+'/'+room_str+'/*.npy'))
                # bridge_paths = bridge_paths[0:10]
                # bridge_paths = sorted(glob.glob(bridge_root2+'/'+area_str+'/'+room_str+'/*.npy')+glob.glob(bridge_root+'/'+area_str+'/'+room_str+'/*.npy'))
                ...
                for cidx in cls_gt_now:
                    incognita = (sam_label_ori != cidx)
                    # print(incognita.shape)
                    # print((seg_pred == cidx).shape)
                    # print((confidence > 0.95).shape)
                    cls_mask_cand = np.logical_and((seg_pred == cidx), (confidence > 0.95)) & incognita.squeeze(1)  # N
                    
                    while np.sum(cls_mask_cand) > 0:
                        cls_mask_cand_idx = np.where(cls_mask_cand)[0]
                        confidence_cls = confidence[np.where(cls_mask_cand)[0]]
                        coord_cls = coord[np.where(cls_mask_cand)[0],:]
                        max_prompt = cls_mask_cand_idx[np.argmax(confidence_cls)]
                        prompt_cls.append(cidx)
                        prompt_idx.append(max_prompt)
                        filter_cls_idx = cls_mask_cand_idx[np.where(np.linalg.norm((coord_cls - coord_cls[np.argmax(confidence_cls),:]), axis = 1) <= radius_scale)[0]]
                        cls_mask_cand[filter_cls_idx] = False
                ...
                # prompt_idx = []
                # grid_scale = 1
                # for xidx in range(int(math.ceil(length_x)//grid_scale)):
                #     start_x = min_x + (xidx)*grid_scale
                #     end_x = min_x + (xidx+1)*grid_scale
                #     mask_x = (coord[:,0]>start_x)*(coord[:,0]<end_x)
                #     for yidx in range(int(math.ceil(length_y//grid_scale))):
                ...
                if prompt_idx.shape[0]>0:
                    ...
                    for bidx, bridge_path in enumerate(bridge_paths):
                        ...
                        if prompt_viewable.sum()>0:
                            ...
                            # if bridge_path.split('/')[-4]== 'bridge':
                            #     emb_path = embedding_root + '/' + area_str + '/' + room_str + '/' + bridge_path.split('/')[-1][:-4] + '.pth' 
                            # elif bridge_path.split('/')[-4]== 'bridge2':
                            #     emb_path = embedding_root2 + '/' + area_str + '/' + room_str + '/' + bridge_path.split('/')[-1][:-4] + '.pth'          
                            ...
                            self.predictor.input_size = (1024,1024)
                            ...
                            for midx, m in enumerate(masks):
                                ...
                                # print(coord_viewable.shape)
                                # print(np.max(coord_viewable))
                                # import pdb;pdb.set_trace()
                                # sam_enhanced_idx = mask_now[coord_viewable[idx_viewable][:,1]-1, coord_viewable[idx_viewable][:,0]-1] # H X W check
                                sam_enhanced_idx = mask_now[coord_viewable[idx_viewable][:,0]-1, coord_viewable[idx_viewable][:,1]-1]
            ...
            if self.epoch==0:
                # 0.004
                self.writer.add_scalar('sam_label/mIoU', 0.304, -1)
                self.writer.add_scalar('sam_label/mPre', 0.831, -1)
                self.writer.add_scalar('sam_label/mRec', 0.328, -1)
                
                # 0.02
                # self.writer.add_scalar('sam_label/mIoU', 0.508, -1)
                # self.writer.add_scalar('sam_label/mPre', 0.848, -1)
                # self.writer.add_scalar('sam_label/mRec', 0.564, -1)
                
                # render_1
                # self.writer.add_scalar('sam_label/mIoU', 0.286, -1)
                # self.writer.add_scalar('sam_label/mPre', 0.937, -1)
                # self.writer.add_scalar('sam_label/mRec', 0.294, -1)
                
                # # render_2
                # self.writer.add_scalar('sam_label/mIoU', 0.325, -1)
                # self.writer.add_scalar('sam_label/mPre', 0.927, -1)
                # self.writer.add_scalar('sam_label/mRec', 0.337, -1)
    ...
    def build_model(self):
        ...
        #
        # temp = torch.load("/home/vilab/khj/ssd0/pointcept/exp/s3dis/sam_0.02_lv0/model/model_best.pth")['state_dict'] # 0.02 sam best
        temp = torch.load("/mnt/jihun4/pointceptHJ/exp/s3dis/semseg-pt-v2m2-0-sam-level-0/model/model_best.pth")['state_dict'] # ours sam best
        sd = {}
        for k,v in temp.items():
            sd[k[7:]] = v.cpu() # remove "module.""
    ...
    def build_train_loader(self):
        ...
        self.cfg.data.train.weak_path = '/mnt/jihun4/pointceptHJ/'+self.cfg.save_path+'/sam_labels_on_the_fly'
        if comm.is_main_process():
            # os.system("cp -r /home/vilab/khj/ssd0/pointcept/data/sam_labels_0.02 " + self.cfg.data.train.weak_path)
            os.system("cp -r /mnt/jihun4/pointceptHJ/data/sam_labels " + self.cfg.data.train.weak_path)
